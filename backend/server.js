...
const { storeRouter, adminRouter: adminShippingRoutes } = require("./src/routes/shipping.routes");
...
// Images are loaded by the frontend from another origin/port.
app.use(helmet({ crossOriginResourcePolicy: { policy: "cross-origin" } }));

// FRONTEND_URL supports a comma-separated list, e.g.
// "https://al-dalouaa.com,https://www.al-dalouaa.com".
// If it's not set, every origin is allowed (fine for local development).
...
app.use(
  "/uploads",
  express.static(UPLOADS_DIR, { maxAge: "7d", index: false, dotfiles: "ignore" })
);
...
app.get("/", (req, res) => {
  res.json({ success: true, message: "AL-DALOUAA API is running 🚀" });
});

app.get("/api/health", async (req, res) => {
  try {
    await prisma.$queryRaw`SELECT 1`;
    res.json({ success: true, message: "API and Database are connected successfully ✅" });
  } catch (error) {
...
// ==============================
// 404 (any request that didn't match a route above)
// ==============================
...
app.use((err, req, res, next) => {
  ...
  res.status(status).json({
    success: false,
    message: status < 500 || !isProduction ? err.message || "Internal server error" : "Internal server error",
  });
});
...
const server = app.listen(PORT, () => {
  console.log(`🚀 AL-DALOUAA API running on http://localhost:${PORT}`);
  console.log(
    isCloudinary
      ? "🖼️  Images: Cloudinary"
